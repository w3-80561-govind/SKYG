  ...
  user_blog_id_fk = request.params.id;
    ...
    connection.execute(statement,[ user_id_fk , user_blog_id_fk ],(error,result)=>{
        ...
        if(error == null || error == undefined){
          if(result.length != 0){
            var statement2 = `update user_actions set user_action_liked = 1 , user_action_disliked = 0 where user_id_fk = ? and user_blog_id_fk = ?`
            ...
            var statement2 = `insert into user_actions(user_action_liked,user_id_fk,user_blog_id_fk) values (1,?,?)`
